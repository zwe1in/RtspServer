...
RtpSink::RtpSink(UsageEnvironment* env, MediaSource* mediaSource, int playloadType)
    : mMediaSource(mediaSource), mSendPacketCallback(NULL), mEnv(env),
    mCsrcLen(0), mExtension(0), mPadding(0), mVersion(RTP_VERSION),
    mPlayloadType(playloadType), mMarker(0), mSeq(0), mTimestamp(0), mTimerId(0)
{
...
void RtpSink::sendRtpPacket(RtpPacket* packet)
{
    ...
    rtpHead->playloadType = mPlayloadType;
...
{
    mTimerId = mEnv->scheduler()->addTimerEventRunEvery(mTimerEvent, ms);
}
